...
# Start server
sudo ip netns exec ns-server sudo -u "$USER" "$QPERF_BIN" server --tls-cert "$SERVER_CRT" --tls-key "$SERVER_KEY" &
SERVER_PID=$!
...
# Migrate client
sleep 3
sudo ip netns exec ns-client ip addr add 10.0.0.102/24 dev eth-client
sudo ip netns exec ns-client ip addr del 10.0.0.101/24 dev eth-client
