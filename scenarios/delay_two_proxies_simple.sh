...
sudo ip netns exec ns-server-side-proxy sudo -u "$USER" "$QPERF_BIN" proxy --tls-cert "$PROXY_CRT" --tls-key "$PROXY_KEY" --client-facing-initial-congestion-window "$INITIAL_CONGESTION_WINDOW" --log-prefix "server_side_proxy" --qlog-prefix "server_side_proxy" $QLOG &
