...
sudo ip netns exec ns-server-side-proxy $QPERF_BIN proxy --tls-cert ../proxy.crt --tls-key ../proxy.key --client-facing-initial-congestion-window $CONGESTION_WINDOW --client-facing-max-congestion-window $MAX_IN_FLIGHT --log-prefix "server_side_proxy" --qlog-prefix "server_side_proxy" $QLOG &
