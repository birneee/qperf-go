...
sudo ip netns exec ns-server-proxy $QPERF_BIN proxy --tls-cert ../proxy.crt --tls-key ../proxy.key --client-side-min-congestion-window 8000 --client-side-max-congestion-window 8000 --client-side-initial-receive-window 50MB --log-prefix "server_side_proxy" &
SERVER_SIDE_PROXY_PID=$!

# Start client-side proxy
sudo ip netns exec ns-client-proxy $QPERF_BIN proxy --tls-cert ../proxy.crt --tls-key ../proxy.key --next-proxy $SERVER_SIDE_PROXY_IP --0rtt --next-proxy-cert ../proxy.crt --server-side-initial-receive-window 50MB --log-prefix "client_side_proxy" &
