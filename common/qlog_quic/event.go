...
// mostly copied from quic-go/qlog/event.go
...
var _ qlog.EventDetails = eventPacketSent{}
...
var _ qlog.EventDetails = eventPacketReceived{}
...
type eventTransportParameters struct {
	Restore bool
	Owner   owner
	SentBy  logging.Perspective

	OriginalDestinationConnectionID logging.ConnectionID
	InitialSourceConnectionID       logging.ConnectionID
	RetrySourceConnectionID         *logging.ConnectionID

	StatelessResetToken     *logging.StatelessResetToken
	DisableActiveMigration  bool
	MaxIdleTimeout          time.Duration
	MaxUDPPayloadSize       logging.ByteCount
	AckDelayExponent        uint8
	MaxAckDelay             time.Duration
	ActiveConnectionIDLimit uint64

	InitialMaxData                 logging.ByteCount
	InitialMaxStreamDataBidiLocal  logging.ByteCount
	InitialMaxStreamDataBidiRemote logging.ByteCount
	InitialMaxStreamDataUni        logging.ByteCount
	InitialMaxStreamsBidi          int64
	InitialMaxStreamsUni           int64

	PreferredAddress *preferredAddress

	MaxDatagramFrameSize logging.ByteCount
}
...
func (e eventTransportParameters) MarshalJSONObject(enc *gojay.Encoder) {
	if !e.Restore {
		...
		if e.SentBy == logging.PerspectiveServer {
	...
	if e.MaxDatagramFrameSize != InvalidByteCount {
